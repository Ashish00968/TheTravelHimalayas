# The Travel Himalayas — Launch Plan, SEO & 1-Year Roadmap

_Last updated: June 2026_

This document is the single strategic reference for taking **The Travel Himalayas**
(`thetravelhimalayas.com`) from its current state to a published, professional,
...
> Decision needed from you: confirm the brand name **The Travel Himalayas** and domain
> **thetravelhimalayas.com**. I standardized the whole codebase on this (it was split
...
**Fixed in this pass**
- ✅ Brand/domain unified to The Travel Himalayas (was split with "TheHimalayanTrails").
...
- [ ] **Domain & hosting** — buy `thetravelhimalayas.com`, deploy on Vercel (best fit for Next.js). Set `www` → apex redirect.
- [ ] **OG image** — create `public/og-default.jpg` (1200×630) with logo + tagline + a hero shot. Per-page OG images later.
- [ ] **Favicon set** — replace default `favicon.ico` with a branded set (see §7).
- [ ] **Analytics** — add Google Analytics 4 + Vercel Analytics. Verify in **Google Search Console** and submit the sitemap (`/sitemap.xml`).
- [ ] **Email provider** — connect the newsletter route to Resend / ConvertKit / Buttondown (wiring point is documented in `src/app/api/newsletter/route.ts`).
- [ ] **Legal pages** — add `/privacy` and `/terms` (mandatory for ad networks & affiliates).
- [ ] **Real contact details** — confirm `hello@thetravelhimalayas.com` mailbox exists.
- [ ] **Content QA** — proofread all data files; verify altitudes, permit rules, prices are current.
- [ ] **Performance pass** — run Lighthouse; target 90+ on all four metrics.
- [ ] **Accessibility pass** — full keyboard nav, color contrast, alt text on every image.
...
1. **Instagram (primary)** — handle suggestion: `@thetravelhimalayas`.
...
**Consistency rules:** same handle everywhere (`@thetravelhimalayas`), same logo/avatar,
...
- [ ] Confirm brand name + buy the domain.
- [ ] Create `public/og-default.jpg` and a real favicon/logo set.
- [ ] Create Instagram + Pinterest accounts; put the URLs in `src/lib/site.ts`.
- [ ] Connect the newsletter route to an email provider.
- [ ] Add `/privacy` and `/terms` pages.
- [ ] Deploy to Vercel; verify Search Console; submit the sitemap.
- [ ] Start the content sprint: expand to 15 treks + 20 guides (Q1 goal).
- [x] Redesigned the Contact Us page using Stitch AI with a cinematic glassmorphism layout and full Framer Motion animations.
